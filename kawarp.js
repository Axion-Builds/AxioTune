...
const DOMAIN_WARP_SHADER = `
  ...
  void main() {
    ...
    vec2 p = uv - 0.5;
    float angle = sin(t * 0.3) * 0.25;
    mat2 rot = mat2(cos(angle), -sin(angle), sin(angle), cos(angle));
    p = rot * p;
    vec2 uvRot = p + 0.5;

    // 3 Octaves of Simplex Domain Warping
    float n1 = snoise(uvRot * 0.7 + vec2(t * 0.8, t * 0.5));
    float n2 = snoise(uvRot * 0.7 + vec2(-t * 0.6, t * 0.7) + vec2(43.0, 17.0));

    float n3 = snoise(uvRot * 1.6 + vec2(t * 1.2, -t * 0.9) + vec2(100.0, 20.0));
    float n4 = snoise(uvRot * 1.6 + vec2(-t * 1.0, t * 1.1) + vec2(20.0, 100.0));

    float n5 = snoise(uvRot * 3.0 + vec2(t * 1.8, t * 1.5) + vec2(70.0, 90.0));
    float n6 = snoise(uvRot * 3.0 + vec2(-t * 1.6, -t * 1.7) + vec2(12.0, 34.0));

    vec2 warp = vec2(
      n1 * 0.50 + n3 * 0.35 + n5 * 0.15,
      n2 * 0.50 + n4 * 0.35 + n6 * 0.15
    );

    vec2 warpedUV = uv + warp * (u_intensity * 0.45);
    // Smooth bounce wrap to eliminate clamping artifacts
